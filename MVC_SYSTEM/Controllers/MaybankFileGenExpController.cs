...
namespace MVC_SYSTEM.Controllers
{
    ...
    public class MaybankFileGenExpController : Controller
    {
        ...
        public FileStreamResult MaybankFIleGenPdf(string CompCodeList, int? MonthList, int? YearList, string print, string PaymentDate)
        {
            ...
            if (maybankDetails.Count() > 0)
            {
                ...
                chunk = new Chunk("Date: " + PostingDate, FontFactory.GetFont("Arial", 8, Font.NORMAL, BaseColor.BLACK));
                cell = new PdfPCell(new Phrase(chunk));
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.Border = 0;
                ...
                chunk = new Chunk(" : " + ClientID, FontFactory.GetFont("Arial", 8, Font.NORMAL, BaseColor.BLACK));
                cell = new PdfPCell(new Phrase(chunk));
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.Border = 0;
