...
namespace MVC_SYSTEM.Controllers
{
    ...
    public class MaybankFileGenController : Controller
    {
        ...
        public ActionResult DownloadTextTaxCP39Prev(int Month, int Year, string CompCode, string filter, string[] WorkerId, DateTime PaymentDate)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
        ...
        public JsonResult CheckGenDataDetailCP39Prev(int Month, int Year, string CompCode, /*int Wilayah,*/ string[] WorkerId)
        {
            string msg = "";
            string statusmsg = "";
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            ...
            if (maybankrcmsList.Count() != 0)
            {
                TotalGaji = maybankrcmsList.Sum(s => s.fld_MTDAmount);
                CountData = maybankrcmsList.Count();
                msg = GlobalResCorp.msgDataFound;
                statusmsg = "success";
            }
        ...
        public ActionResult TaxCP39PrevGen()
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
        ...
        public ViewResult _taxcp39prev(string CompCodeList, int? MonthList, int? YearList, string print, string filter)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
            ...
            //    .Select(s => s.fld_LdgCode)
            //    .FirstOrDefault();
            //ViewBag.Ladang = LdgName.Trim();DownloadText
            //ViewBag.LadangCode = LdgCode.Trim();
            //ViewBag.OriginatorId = LdgDetail.fld_OriginatorID;
            //ViewBag.OriginatorName = LdgDetail.fld_OriginatorName;
            //ViewBag.AccNo = LdgDetail.fld_NoAcc;
            ...
            {
                dbSP.SetCommandTimeout(2400);
                maybankrcmsList = dbSP.sp_TaxCP39Prev(NegaraID, SyarikatID, YearList, MonthList, getuserid, CompCodeList).ToList();
